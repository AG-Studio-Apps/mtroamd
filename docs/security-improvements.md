...
| **F9** - Daemon-supplied session `Name` / foreground-command strings were written unsanitized to the interactive CLI terminal - control-sequence injection | Low | Claude Security scan (multi-agent) | Shared `sanitizeForDisplay` (C0/C1/`ESC`/`DEL`/invalid-UTF-8 -> U+FFFD) at the structured CLI sinks (list / search / status / session-info / attach header); the raw attach + tail relay path is left untouched (`bdd85e6`). The pre-push `/code-review` then extended coverage to every remaining daemon-supplied sink - `doctor` (the re-run report block, the skew fields, and both error paths), `restart`, and the daemon-`stderr` error paths of `kill`/`new`/`rename`/`attach` - via a line-preserving `sanitizeMultilineForDisplay`, and hardened the tabwriter cells with `sanitizeForCell` so a TAB embedded in a daemon-supplied field can no longer inject a spurious column and spoof the table (`798415f`) |
| **F10** - `mtroam search` rendered raw scrollback bytes (escape sequences) to the local terminal without neutralizing them | Low | Claude Security scan (multi-agent) | Same sanitization, now via `sanitizeForCell` (`bdd85e6`, `798415f`) |
