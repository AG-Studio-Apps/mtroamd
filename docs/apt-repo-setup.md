...
```sh
gpg --batch --quick-generate-key "mtroamd apt repo" ed25519 sign never
...
```sh
gpg --export "mtroamd apt repo" > docs/apt-signing-key.gpg
gpg --armor --export "mtroamd apt repo" > docs/apt-signing-key.asc
```
...
touch .nojekyll
cp /path/to/mtroamd-archive-keyring.gpg .      # served at the Pages root
...
Confirm `https://ag-studio-apps.github.io/mtroamd/mtroamd-archive-keyring.gpg`
serves the key.
...
`sudo apt install mtroamd`. Once 2.0 ships, a clean `vX.Y.Z` tag on `main`
publishes to `stable`.
