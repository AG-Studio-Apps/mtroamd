...
# Prior ~/.local/bin install → migrate onto the package binary. Trigger ONLY
# when there is genuinely something to migrate: the old binary still exists, OR
# the active unit still launches mtroamd from ~/.local/bin. The unit FILE lives
# at $old_unit for package installs too, so its mere presence is NOT a migration
# signal — keying on it re-ran a no-op `migrate` on every upgrade and exited
# before the upgrade restart below, leaving the new binary on disk while the old
# one kept running (until a manual `mtroamd restart`).
