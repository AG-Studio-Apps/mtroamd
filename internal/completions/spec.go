...
var updateFlags = []Flag{
	...
	{Long: "--force", Help: "reinstall the target tag even if it matches the running version"},
}
...
var Mtroam = Binary{
	...
	Subcommands: []Subcommand{
		...
		{Name: "attach", Help: "attach to a session as your local terminal",
			Flags: []Flag{hostFlag,
				{Long: "--mode", Help: "attach mode: exclusive (default), readonly, or exclusive-if-free"},
				...
				{Long: "--name", Help: "with `attach new`, name for the fresh session"},
				{Long: "--idle-timeout", Help: "idle GC timeout for a fresh spawn (e.g. 1h, 7d)"},
				{Long: "--shell", Help: "with `attach new`, override the remote $SHELL"},
