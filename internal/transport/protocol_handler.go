...
func (h *ProtocolHandler) HandleConnection(ctx context.Context, ctrl Conn) {
	...
	// ★★ STRANDED ALT SCREEN — trim the window, do NOT inject a recovery byte.
	//
	// A TUI killed ungracefully (SIGKILL, host reboot, a daemon restart that ate
	// its exit) never emits ?1049l, so the ring keeps its ENTER forever. Measured
	// on a real session: 2x ESC[?1049h and zero ESC[?1049l, the last enter 16 KiB
	// before head. Every attach replayed it, so the client came up on the alt
	// buffer under a plain bash — and an iOS client that believes it is on the alt
	// screen routes pans to its mouse-wheel forwarder, killing scrollback.
	//
	// ★ The first attempt at this appended a synthetic ESC[?1049l so it would ride
	// the tail of the replay. Review killed it, correctly. The client would still
	// parse the enter, paint EVERYTHING after it onto the alt buffer, and then the
	// exit would discard that buffer: on the measured session those 16 KiB held
	// the user's `ls` output and their live prompt. It also wrote to the SHARED
	// ring, so outputPump broadcast a bare ?1049l — which carries a cursor restore
	// — to every already-attached client.
	//
	// Trimming has none of that. The client never sees the enter, never switches
	// buffers, and receives the post-enter bytes on the MAIN screen where they
	// belong. Nothing is written to the ring, so co-attached clients are untouched
	// and the session's history is not rewritten. `offsetPastEnter` is measured
	// past a CSI final byte, so the new start can never slice an escape in half.
	//
	// Gated on a FRESH AltScreenForClient read (the one taken at the top of the
	// attach is ~100 lines stale by here, and a TUI can enter alt during the
	// handshake), or on wasRestored — a lazy-spawned restore is a brand-new shell,
	// which cannot be on the alt buffer, and whose fg poller reads "" for its
	// first seconds. That `wasRestored` term is why the mouse twin above has one.
	if !fullRedraw && att.ReplayBudget > 0 && head > start &&
		(wasRestored || !sess.AltScreenForClient()) {
		if newStart, trimmed := trimStrandedAltScreen(buf, start, head); trimmed {
			start, trunc = newStart, true
			slog.Info("attach.altstrand", "sid", sess.ID().String(),
				"trimmedTo", start, "head", head,
				"msg", "replay window ended on a stranded alt-screen enter; trimmed past it")
		}
	}
...
// trimStrandedAltScreen returns the replay start advanced past a stranded
// alt-screen enter, when the window [start, head) would otherwise leave the
// client on the alternate buffer. Reports whether it moved.
//
// Split out from HandleConnection so the seq arithmetic is directly testable:
// the property that matters is not "we detected it" but "the window the client
// actually replays leaves them on the MAIN buffer with the post-enter content
// intact", and that is a statement about the returned start.
func trimStrandedAltScreen(buf *session.RingBuffer, start, head uint64) (uint64, bool) {
	if buf == nil || head <= start {
		return start, false
	}
	win, winEnd, _ := buf.ReadSince(start, int(head-start))
	if len(win) == 0 {
		return start, false
	}
	onAlt, past := altScreenScan(win)
	if !onAlt {
		return start, false
	}
	// winEnd - len(win) is the seq the returned data STARTS at, which is NOT
	// `start` when the ring tail moved under us mid-attach.
	return winEnd - uint64(len(win)) + uint64(past), true
}

// altScreenScan walks a replay window and reports whether those bytes would
// leave a client's emulator on the ALTERNATE screen, and if so the byte offset
// immediately past the sequence that put it there.
//
// ★ Why a real scanner and not bytes.LastIndex. The first cut of this used
// substring search, and review found three ways that misreads a stream:
// RIS (`ESC c`, what `reset` emits) returns the terminal to the main buffer
// without ever emitting ?1049l; a DECSET/DECRST may carry SEVERAL parameters
// (`ESC[?1049;1002h`), so the mode is not adjacent to the final byte; and the
// literal bytes `1049h` can appear inside an OSC or DCS payload — a window
// title, a prompt echoing an escape — where they are data, not commands. Each
// produced a spurious verdict on a healthy main-buffer session.
//
// One forward pass, no allocation: cheaper than the six LastIndex scans it
// replaces. It cannot be bounded to the tail — an enter anywhere in the window
// decides, and only a later toggle can undo it.
func altScreenScan(win []byte) (endsOnAlt bool, offsetPastEnter int) {
	n := len(win)
	for i := 0; i < n; {
		if win[i] != 0x1B {
			i++
		...
		if i+1 >= n {
			break // escape truncated at the window edge
		}
		switch win[i+1] {
		case '[': // CSI
			j := i + 2
			var private byte
			if j < n && (win[j] == '?' || win[j] == '>' || win[j] == '=') {
				private = win[j]
				j++
			}
			ps := j
			for j < n && (isCSIParamByte(win[j]) || isCSIIntermediate(win[j])) {
				j++
			}
			if j >= n {
				return endsOnAlt, offsetPastEnter // sequence runs off the end
			}
			final := win[j]
			if private == '?' && (final == 'h' || final == 'l') && hasAltScreenMode(win[ps:j]) {
				endsOnAlt = final == 'h'
				if endsOnAlt {
					// Past the final byte, so a trim here always lands on a
					// sequence boundary and can never slice an escape in half.
					offsetPastEnter = j + 1
				}
			}
			i = j + 1
		case ']': // OSC — payload is data; BEL or ST terminated
			i = skipStringPayload(win, i+2)
		case 'P', '_', '^', 'X': // DCS / APC / PM / SOS — ST terminated
			i = skipStringPayload(win, i+2)
		case 'c': // RIS — a full reset returns the terminal to the main buffer
			endsOnAlt = false
			i += 2
		default:
			i += 2
	...
	return endsOnAlt, offsetPastEnter
}

func isCSIParamByte(b byte) bool  { return (b >= '0' && b <= '9') || b == ';' }
func isCSIIntermediate(b byte) bool { return b >= 0x20 && b <= 0x2F }

// hasAltScreenMode reports whether a DECSET/DECRST parameter list names one of
// the three buffer-switch modes. Handles multi-parameter lists; bails on an
// intermediate byte (DECRQM's `$p` is a QUERY, not a set, and must not count).
func hasAltScreenMode(params []byte) bool {
	v := -1
	for _, b := range params {
		...
		case b >= '0' && b <= '9':
			...
			if v < 1<<20 { // clamp; a pathological run must not overflow
				v = v*10 + int(b-'0')
			}
		case b == ';':
			if isAltScreenMode(v) {
				return true
			}
			v = -1
		default:
			return false // intermediate byte → not a plain mode list
		}
	}
	return isAltScreenMode(v)
}

func isAltScreenMode(v int) bool { return v == 1049 || v == 1047 || v == 47 }

// skipStringPayload returns the offset past a string-terminated payload (OSC,
// DCS, APC, PM, SOS), so its contents are never parsed as commands.
func skipStringPayload(win []byte, i int) int {
	for i < len(win) {
		if win[i] == 0x07 { // BEL (OSC's common terminator)
			return i + 1
		}
		if win[i] == 0x1B && i+1 < len(win) && win[i+1] == '\\' { // ST
			return i + 2
		}
		i++
	}
