...
// runUninstall implements `mtroamd uninstall [--purge] [--yes]`.
//
// Plain `uninstall`:
...
func runUninstall(args []string) int {
	...
	purge := fs.Bool("purge", false,
		"also remove ~/.local/share/mtroamd (cert, private key, sockets). "+
			"Default keeps state so a future reinstall can reuse the same identity.")
	yes := fs.Bool("yes", false, "skip the confirmation prompt")
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), "Usage: mtroamd uninstall [flags]\n\n")
		fs.PrintDefaults()
	}
	_ = fs.Parse(args)
	...
	if release.IsPackageManaged() {
		fmt.Fprintf(os.Stderr, "uninstall: %s\n",
			release.PackageManagedHint("sudo apt remove mtroamd"))
		return 2
	}
	...
	if !*yes {
		...
		fmt.Println("  • Stop any running mtroamd daemon")
	...
	// ran `mtroamd serve &`). Runs AFTER the supervisor stop, and
