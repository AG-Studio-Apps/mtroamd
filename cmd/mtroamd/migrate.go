...
// runMigrate implements `mtroamd migrate` — switch a host's systemd --user
// setup from a prior ~/.local/bin install (placed by the iOS app or
// `mtroamd update`) onto the package-managed binary this command is running
// as (e.g. /usr/bin/mtroamd from the apt package).
//
// State in ~/.local/share/mtroamd (cert, sessions/) is shared by both
...
//     the old unit's --addr/--socket/--mtroam-tcp-addr flags so the daemon
...
func runMigrate(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), "Usage: mtroamd migrate [--yes]\n\n")
		fs.PrintDefaults()
	}
	_ = fs.Parse(args)
	...
	if !release.IsPackageManaged() {
		...
		fmt.Println("Run migrate from the package-managed binary, e.g. `/usr/bin/mtroamd migrate`.")
		return 0
	}

	oldBin := release.JoinBin() // ~/.local/bin/mtroamd
	...
	if !*yes {
		fmt.Println("Migrate this host's mtroamd to the package-managed binary:")
		fmt.Printf("  • %s → %s\n", oldBin, newBin)
		fmt.Println("  • rewrites the systemd --user unit + restarts (live sessions preserved)")
		fmt.Println("  • removes the old binary; shared state in ~/.local/share/mtroamd is kept")
		fmt.Print("\nProceed? [y/N] ")
	...
	rollback := func(stage string, out []byte, err error) int {
		...
		_, _ = svcmgr.SystemctlUser(ctx, "restart", "mtroamd")
		return 5
	}
	...
	if out, err := svcmgr.SystemctlUser(ctx, "enable", "--now", "mtroamd"); err != nil {
	...
	if out, err := svcmgr.SystemctlUser(ctx, "restart", "mtroamd"); err != nil {
		return rollback("restart", out, err)
	}
	if out, err := svcmgr.SystemctlUser(ctx, "is-active", "mtroamd"); err != nil ||
	...
	fmt.Printf("✓ migrated to %s — sessions preserved. Updates now via: sudo apt upgrade mtroamd\n", newBin)
	return 0
}

// parseExecStartFlags pulls --addr/--socket/--mtroam-tcp-addr out of the old
...
// an absent --mtroam-tcp-addr is preserved as the "-" (QUIC-only) sentinel.
